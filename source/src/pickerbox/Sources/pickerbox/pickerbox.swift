// swift build --package-path ./src/pickerbox -Xswiftc "-target" -Xswiftc "x86_64-apple-macosx10.15" -Xlinker -rpath -Xlinker @executable_path/Frameworks
...
struct ContentView: View {
    @State private var dropdown_options: [String] = options["items"] as! [String];
    @State private var selected_option = 0;
    var body: some View {
        VStack(spacing: 0) {
            HeaderView()
            BodyView(dropdown_options: $dropdown_options, selected_option: $selected_option)
            FooterView(dropdown_options: $dropdown_options, selected_option: $selected_option)
        }
        .padding(5)
    }
}
...
struct BodyView: View {
    @Binding var dropdown_options: [String];
    ...
    var body: some View {
        HStack(alignment: .top) {
            ...
            MainView(dropdown_options: $dropdown_options, selected_option: $selected_option)
        }
...
struct MainView: View {
    ...
    var body: some View {
        VStack {
            ...
            PickerView(dropdown_options: $dropdown_options, selected_option: $selected_option)
        }
...
struct PickerView: View {
    @Binding var dropdown_options: [String];
    @Binding var selected_option: Int;
    var styleOption = options["style"] as? String ?? "default";

    var body: some View {
        VStack {
            if (styleOption == "radio") {
                Picker("", selection: $selected_option) {
                    ForEach(0 ..< dropdown_options.count) {
                        Text(self.dropdown_options[$0])
                        .font(.body)
                    }
                }
                .labelsHidden()
...
struct FooterView: View {
    ...
    var body: some View {
        HStack() {
            ...
            ButtonsView(dropdown_options: $dropdown_options, selected_option: $selected_option)
        }
    }
}
