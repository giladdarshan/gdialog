// swift build --package-path ./src/fileselect -Xswiftc "-target" -Xswiftc "x86_64-apple-macosx10.15" -Xlinker -rpath -Xlinker @executable_path/Frameworks
...
struct ContentView: View {
    ...
    @State private var file_selection_resulsts: [String] = [];

    var body: some View {
        VStack(spacing: 0) {
            HeaderView()
            BodyView(file_path: $file_path, file_selection_resulsts: $file_selection_resulsts)
            FooterView(file_path: $file_path, file_selection_resulsts: $file_selection_resulsts)
        }
        .padding(5)
    }
}
...
struct BodyView: View {
    ...
    @Binding var file_selection_resulsts: [String];
    ...
    var body: some View {
        HStack(alignment: .top) {
            ...
            MainView(file_path: $file_path, file_selection_resulsts: $file_selection_resulsts)
        }
...
struct MainView: View {
    ...
    var body: some View {
        VStack {
            ...
            FileSelectView(file_path: $file_path, file_selection_resulsts: $file_selection_resulsts)
        }
...
struct FileSelectView: View {
    ...
    func openPanel() {
        ...
        if (options["select_multiple"] as! Bool) {
            dialog.allowsMultipleSelection = true;
        }
        else {
            dialog.allowsMultipleSelection = false;
        }
        if (options["select_only_directories"] as! Bool) {
            dialog.canChooseFiles = false;
        }
        else {
            dialog.canChooseFiles = true;
        }
        if (options["select_directories"] as! Bool) {
        ...
        if (dialog.runModal() ==  NSApplication.ModalResponse.OK) {
            ...
            file_selection_resulsts.append(results[0].path)
            if results.count > 1 {
                for i in 1..<results.count {
                    file_path += ",\(results[i].path)"
                    file_selection_resulsts.append(results[i].path)
                }
            }
        }
    }
...
struct FooterView: View {
    ...
    var body: some View {
        HStack() {
            ...
            ButtonsView(file_path: $file_path, file_selection_resulsts: $file_selection_resulsts)
        }
    }
}
    ...
    var body: some View {
        HStack {
            ForEach(options["buttons"] as! [String], id: \.self) {
                ButtonView(
                    file_path: $file_path,
                    file_selection_resulsts: $file_selection_resulsts,
                    labelText: "\($0)",
...
struct ButtonView: View {
    ...
    func buttonClicked() {
        var returnText = actionReturn;
        file_selection_resulsts.forEach { path in
            returnText += "\n\(path)";
        }
